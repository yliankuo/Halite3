...
class PriorityQueue:
    ...
    def put(self, item, priority):
        if str(item) in self.entry_finder:
            self.rem(str(item))
        count = next(self.counter)
        entry = [priority, count, item]
        self.entry_finder[str(item)] = entry
        heapq.heappush(self.elements, entry)

    def get(self):
        while self.elements:
            prior, count, item =  heapq.heappop(self.elements)
            if item is not self.REMOVED:
                if str(item) in self.entry_finder:
                    del self.entry_finder[str(item)]
                return item
...
class GameMap:
    ...
    def get_total_halite(self,pos = None,inspire = False):
        total = 0
        ranks = PriorityQueue()
        ranksbyhalite = []
        for row in self._cells:
            for cell in row:
                ...
                ranks.put(cell,-1 * cell.halite_amount)
        while not ranks.empty():
            ranksbyhalite.append(ranks.get())
        return total, ranksbyhalite
